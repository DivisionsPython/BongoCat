...
from utils.subclasses import ClassicDetailedEmbed
...
class MyHelpCommand(commands.MinimalHelpCommand):
    ...
    async def _help_embed(
        ...
        embed = ClassicDetailedEmbed(self.context.author)
        embed.title = title
        if description:
            embed.description = description
        if set_author:
            avatar = self.context.bot.user.avatar or self.context.bot.user.default_avatar
            embed.set_author(name=f'{self.context.bot.user.name} help command',
                             icon_url=avatar.url)
        if command_set:
